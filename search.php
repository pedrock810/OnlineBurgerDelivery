...
<head>
    ...
    <title>Home</title>
    <link rel = "icon" href ="img/logotip.jpg" type = "image/x-icon">
    <style>
    #cont {
        min-height : 515px;
    }
...
<body>
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/categoria-'.$catId. '.jpg" class="card-img-top" alt="image for this burger" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title"><a href="viewBurgerList.php?catid=' . $catId . '">' . $catname . '</a></h5>
                            <p class="card-text">' . substr($catdesc, 0, 29). '...</p>
                            <a href="viewBurgerList.php?catid=' . $catId . '" class="btn btn-primary">View All</a>
                        </div>
                    </div>
                </div>';
            }
    ...
    <div class="container my-3" id="cont">
        ...
        <?php 
            $query = $_GET["search"];
            $sql = "SELECT * FROM `burger` WHERE MATCH(burgerName, burgerDesc) against('$query')"; 
            $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/burger-'.$burgerId. '.jpg" class="card-img-top" alt="image for this burger" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title">' . substr($burgerName, 0, 20). '...</h5>
                            <h6 style="color: #ff0000">Rs. '.$burgerPrice. '/-</h6>
                            <p class="card-text">' . substr($burgerDesc, 0, 29). '...</p>
                            <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `itemQuantity` FROM `viewcart` WHERE burgerId = '$burgerId' AND `userId`='$userId'";
                                    $quaresult = mysqli_query($conn, $quaSql);
                                    $quaExistRows = mysqli_num_rows($quaresult);
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              <input type="hidden" name="itemId" value="'.$burgerId. '">
                                ...
                                echo '</form>
                                <a href="viewBurger.php?burgerid=' . $burgerId . '"><button class="btn btn-primary">Quick View</button></a>
                            </div>
                        </div>
                    </div>
                </div>';
